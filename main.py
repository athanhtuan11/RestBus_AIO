...
for i, (label, var, btn) in enumerate(labels):
    ttk.Label(mainframe, text=label).grid(row=i, column=0, sticky='e', padx=3, pady=3)
    if label == 'Channel:':
        channel_combobox = ttk.Combobox(mainframe, textvariable=Channel_var, values=["PUB", "PRI"], state="readonly")
        channel_combobox.grid(row=i, column=1, sticky='ew', padx=3, pady=3)
        channel_combobox.set("PUB")
    else:
        ttk.Entry(mainframe, textvariable=var).grid(row=i, column=1, sticky='ew', padx=3, pady=3)
